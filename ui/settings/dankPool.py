...
async def update_pool_embed(interaction: Interaction, data: dict):
    ...
    embed = discord.Embed(color=3092790, title="Dank Pool Access")
    embed.add_field(
        name="Following users are whitelisted:", value=f"{desc}", inline=False
    )
    ...
    if event_manager_error != "":
        embed.add_field(
            name="<a:nat_warning:1062998119899484190> Warning: <a:nat_warning:1062998119899484190>",
            value=f"{event_manager_error}",
            inline=False,
        )
...
class Dank_Pool_Panel(discord.ui.View):
        ...
        if view.value is None:
            ...
            if data["logs_channel"] is None or data["logs_channel"] != view.value.id:
                ...
                embed = await get_success_embed(
                    f"Logging Channel changed from {channel} to {view.value.mention}"
                )
                ...
                embed = await get_error_embed(
                    f"Logging Channel was already set to {channel}"
                )
                return await interaction.edit_original_response(
                    content=None, embed=embed, view=None
                )
    ...
    async def delete_user(
        ...
        data = await interaction.client.dankSecurity.find(interaction.guild.id)
        users = [
            interaction.guild.get_member(int(user_id)) for user_id in data["whitelist"]
        ]
        options = []
        ...
        for user in users:
            options.append(
                discord.SelectOption(
                    label=f"{user.name} {user.name}#{user.discriminator}",
                    value=user.id,
                    emoji="<:tgk_removePerson:1132593125588733983>",
                )
            )
    ...
    async def modify_role(
        ...
        view.select = Role_select(
            placeholder="Select the role for Pool Access", min_values=1, max_values=1
        )
        ...
        if data["event_manager"] == role.id:
            embed = await get_error_embed(
                f"Pool Access role was already set to {role.mention}"
            )
            return await view.select.interaction.response.edit_message(
                content=None, embed=embed, view=None
            )
        else:
            data["event_manager"] = role.id
            await interaction.client.dankSecurity.upsert(data)
            embed = await get_success_embed(
                f"Pool Access role changed to {role.mention}"
            )
...
class Dank_Pool_Whitelist_User(discord.ui.View):
    ...
    @discord.ui.select(
        cls=UserSelect,
        placeholder="Whom do you wish to whitelist?",
        min_values=1,
        max_values=1,
    )
    async def select_channels(
        ...
        if user.id not in data["whitelist"]:
            ...
            await interaction.client.dankSecurity.upsert(data)
            embed = await get_success_embed(
                f"{user.mention} has complete access to Dank Server Pool."
            )
            await update_pool_embed(self.interaction, data)
            ...
            embed = await get_error_embed(
                f"{user.mention} already has access to Dank Server Pool."
            )
            await interaction.response.edit_message(
                content=None,
                embed=embed,
...
class Dank_Pool_Remove_Whitelist_User(discord.ui.Select):
    def __init__(self, interaction: discord.Interaction, options):
        super().__init__(
            placeholder="Whom do you wish to unwhitelist?",
            custom_id="REMOVE:WHITELISTED:USER",
            options=options,
        )
        self.interaction = interaction
        self.message = None
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await update_pool_embed(self.interaction, data)
        embed = await get_success_embed(
            f"Revoked {user.mention}'s access to Dank Server Pool."
        )
        await interaction.response.edit_message(
            content=None,
            embed=embed,
...
class Dank_Pool_Quarantine_Role(discord.ui.View):
    ...
    @discord.ui.select(
        cls=RoleSelect,
        placeholder="Select quarantine role for this server...",
        min_values=1,
        max_values=1,
    )
    async def select_channels(
        ...
        if role.position >= interaction.guild.me.top_role.position:
            embed = await get_error_embed(
                f"Quarantine role cannot be higher than my highest role {interaction.guild.me.top_role.mention}."
            )
            await interaction.response.edit_message(
                content=None,
                embed=embed,
        ...
        if data["quarantine"] is None or role.id != int(data["quarantine"]):
            embed = await get_success_embed(
                f"Updated quarantine role from <@&{data['quarantine']}> to {role.mention}"
            )
            ...
            embed = await get_error_embed(
                f"Quarantine role was already set to {role.mention}"
            )
            await interaction.response.edit_message(
                content=None,
                embed=embed,
