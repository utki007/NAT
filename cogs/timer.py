...
from utils.embeds import get_error_embed, get_warning_embed
...
class Button(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="timer_button",
    )
    async def click_me(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        timer_data = await interaction.client.timer.find(
            {"_id": interaction.message.id}
        )
        if timer_data is None:
            return await interaction.response.send_message(
                "Invalid/Expired timer", ephemeral=True
            )

        if interaction.user.id in timer_data["members"]:
            error_embed = await get_error_embed("You have already entered the timer!")
            return await interaction.response.send_message(
...
class Timer(commands.GroupCog, name="timer", description="Timer commands"):
    ...
    @app_commands.command(
        ...
        extras={"example": "/timer start [time]"},
    )
    @app_commands.checks.cooldown(1, 3, key=lambda i: (i.guild_id, i.user.id))
    @app_commands.describe(
        time="Enter time in format: 1h30m2s", title="Enter title of the timer"
    )
    async def timerStart(
        self, interaction: discord.Interaction, time: str, title: str = "Timer"
    ):
        try:
            ...
            warning = await get_warning_embed(
                "Incorrect time format, please use `1h30m10s`"
            )
            return await interaction.response.send_message(
    ...
    @app_commands.command(
        ...
        extras={"example": "/timer end [timer_id]"},
    )
    @app_commands.checks.cooldown(1, 3, key=lambda i: (i.guild_id, i.user.id))
    ...
    @app_commands.command(
        ...
        extras={"example": "/timer delete [timer_id]"},
    )
    @app_commands.checks.cooldown(1, 3, key=lambda i: (i.guild_id, i.user.id))
    @app_commands.describe(message_id="Enter Message ID of an active timer")
    async def timerDelete(self, interaction: discord.Interaction, message_id: str):
        ...
        try:
            ...
            await interaction.edit_original_response(
                content="Timer deleted successfully!"
            )
        except Exception:
            warning = await get_warning_embed("Invalid timer ID")
            return await interaction.edit_original_response(embed=warning)
    ...
    @app_commands.command(
        name="re-ping",
        description="Ping from an expired timer!",
        extras={"example": "/timer re-ping [timer_id]"},
    )
    @app_commands.checks.cooldown(1, 3, key=lambda i: (i.guild_id, i.user.id))
    ...
    async def timerPing(self, interaction: discord.Interaction, message_id: str):
        ...
        try:
            ...
            if timer_data is None:
                return await interaction.edit_original_response(
                    content="Cannot ping from a timer that has ended more than an hour ago!"
                )
            if timer_data["user_id"] != interaction.user.id:
                return await interaction.edit_original_response(
                    content="You are not the host of the timer!"
            ...
            if timer_data["time"] > datetime.datetime.now():
                return await interaction.edit_original_response(
                    content="Timer has not ended yet!"
                )
            if len(timer_data["members"]) == 0:
                return await interaction.edit_original_response(
                    content="No one has entered the timer!"
                )
            ...
            member_list = [f"<@{int(member)}>" for member in timer_data["members"]]

            try:
                ping_group = list(chunk(member_list, 30))
                for members in ping_group:
                    await message.reply(f"{', '.join(members)}", delete_after=1.5)
            ...
            try:
                ...
                view.add_item(
                    discord.ui.Button(label="Timer link", url=message.jump_url)
                )
                end_message = await message.reply(
                    f"<@{timer_data['user_id']}> your timer has been repinged!",
                    view=view,
                )
    ...
    async def on_timer_end(self, timer_data, sleep: bool = None):
        ...
        if "Timer ended!" in embed.description:
            time_diff_to_delete = int(
                (datetime.datetime.now() - timer_data["time"]).total_seconds()
            )
        ...
        try:
            ...
            for members in ping_group:
                await channel.send(
                    f"{', '.join(f'<@{id}>' for id in members)}", delete_after=1
                )
        except discord.HTTPException:
            pass
        ...
        try:
            view = discord.ui.View()
            view.add_item(discord.ui.Button(label="Timer link", url=message.jump_url))
            if title == "Timer":
                end_message = await channel.send(
                    f"<@{timer_data['user_id']}> your timer has ended!", view=view
                )
            else:
                end_message = await channel.send(
                    f"<@{timer_data['user_id']}> your timer for **{timer_data['title'].title()}** has ended!",
                    view=view,
                )
            await end_message.add_reaction("<a:gk_waiting:945772518776664104>")
        except discord.HTTPException:
    ...
    async def timer_loop_error(self, error):
        ...
        if len(error) > 2000:
            await channel.send(
                file=discord.File(io.BytesIO(error.encode()), filename="error.txt"),
                content="<@301657045248114690>, <@488614633670967307> Timer loop error",
            )
        else:
