...
class adventure(
    ...
    async def round_pfp_4_advtop3(
        self, pfp: discord.User | discord.Member | discord.Guild
    ):
    ...
    async def create_adv_top3(self, data: dict, user_position: int, total_users: int):
        ...
        draw.text(
            xy=(160, 480),
            text="Adventure Leaderboard",
            fill=(0, 0, 1),
            font=ImageFont.truetype("./assets/fonts/DejaVuSans.ttf", 100),
            stroke_width=4,
            spacing=8,
        )
        draw.text(
            xy=(500, 630),
            text=f"Rank: #{user_position}/{total_users}",
            fill=(0, 0, 1),
            font=ImageFont.truetype("./assets/fonts/DejaVuSans.ttf", 81),
            stroke_width=2,
            spacing=8,
        )
        ...
        draw.text(
            ...
            text=data[1]["name"],
            fill=(254, 205, 61),
            font=ImageFont.truetype("./assets/fonts/Symbola.ttf", 100),
            stroke_width=2,
            spacing=10,
        )
        draw.text(
            xy=(680, 1250),
            text=f"⏣ {await millify(data[1]['amount'])}",
            fill=(80, 200, 120),
            font=ImageFont.truetype("./assets/fonts/DejaVuSans.ttf", 100),
            stroke_width=2,
            spacing=10,
        ...
        draw.text(
            ...
            text=data[2]["name"],
            fill=(254, 205, 61),
            font=ImageFont.truetype("./assets/fonts/Symbola.ttf", 75),
            stroke_width=2,
            spacing=10,
        )
        draw.text(
            xy=(155, 2350),
            text=f"⏣ {await millify(data[2]['amount'])}",
            fill=(80, 200, 120),
            font=ImageFont.truetype("./assets/fonts/DejaVuSans.ttf", 75),
            stroke_width=2,
            spacing=10,
        )
        ...
        draw.text(
            xy=(885, 2350),
            text=f"⏣ {await millify(data[3]['amount'])}",
            fill=(80, 200, 120),
            font=ImageFont.truetype("./assets/fonts/DejaVuSans.ttf", 75),
            stroke_width=2,
            spacing=10,
    ...
    async def create_item_lb(self, guild: discord.Guild, data: list):
        ...
        for item in data["items"].keys():
            draw.text(
                xy=(142, 1329 + (int(item) * 100)),
                text=f"{data['items'][item]['qty']}x ",
                fill=(255, 211, 63),
                font=item_font,
                align="right",
                stroke_width=2,
            ...
            draw.text(
                xy=(458, 1329 + (int(item) * 100)),
                text=f"{data['items'][item]['name']} ",
                fill=(255, 255, 255),
                font=item_font,
                align="right",
                stroke_width=1,
            ...
            draw.text(
                xy=(1017, 1329 + (int(item) * 100)),
                text=f"⏣ {data['items'][item]['worth']}",
                fill=(255, 255, 255),
                font=item_font,
                align="left",
                stroke_width=1,
        ...
        draw.text(
            xy=(245, 2048),
            text=f"Dank Adventure stats by: {self.bot.user}",
            fill=(80, 200, 120),
            font=footer_font,
            align="right",
            stroke_width=1,
    ...
    @app_commands.command(
        name="stats", description="Get adventure related statistics 📊"
    )
    @app_commands.checks.cooldown(1, 5, key=lambda i: (i.guild_id, i.user.id))
    async def adventure_stats(
        ...
        if data is None:
            return await interaction.edit_original_response(
                embed=await get_invisible_embed(
                    "You have not played any adventure yet!"
                )
            )
        if today not in data["rewards"].keys():
            return await interaction.edit_original_response(
                embed=await get_invisible_embed("You have not played adventure today!")
            )
        ...
        if len(data["items"]) > 0:
            ...
            for item_name in dict.keys():
                ...
                temp_dict = {
                    "Name": item_name,
                    "Quantity": str(dict[item_name]),
                    "Amount": await millify(amount),
                    "dummy": amount,
                }
            ...
            for index in df.index:
                ...
                quantity = (
                    f'{df["Quantity"][index]}'
                    if int(df["Quantity"][index]) >= 10
                    else f'  {df["Quantity"][index]}'
                )
        ...
        if image is None:
            return await interaction.edit_original_response(
                embed=await get_invisible_embed(
                    "Make sure to comeback after you have grinded more items!"
                )
            )
        ...
        with BytesIO() as image_binary:
            ...
            image_binary.seek(0)
            file = discord.File(fp=image_binary, filename="item_leaderboard.png")
            await interaction.edit_original_response(
                content=f"## Adventure Summary\n- **Showing For:** {user.mention} \n- **Total Grind:** ⏣ {grind:,}",
                attachments=[file],
    ...
    @app_commands.command(
        name="leaderboard", description="Get adventure leaderboard 📊"
    )
    @app_commands.checks.cooldown(1, 5, key=lambda i: (i.guild_id, i.user.id))
    async def adventure_leaderboard(self, interaction: discord.Interaction):
        ...
        for user_record in data:
            ...
            try:
                final_data.append(
                    {"user_id": user_record["_id"], "amount": df["Amount"].sum() + dmc}
                )
            except Exception:
        ...
        if len(items_not_found) > 0:
            ...
            embed = await get_invisible_embed(f"\n1. {items_not_found}")
            embed.title = (
                "</adventure leaderboard:1151588100204658819> : Items not found"
            )
            channel = await interaction.client.fetch_channel(1168987042697449522)
            await channel.send(embed=embed)
        ...
        if None in leaderboard.values():
            return await interaction.followup.send(
                embed=await get_invisible_embed(
                    "Not enough players have played adventure today!"
                )
            )

        image = await self.create_adv_top3(
            data=leaderboard, total_users=len(data), user_position=user_position
        )

        with BytesIO() as image_binary:
            image.save(image_binary, "PNG")
            image_binary.seek(0)
            file = discord.File(fp=image_binary, filename="adventure_leaderboard.png")
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            item_data["last_prices"].append(
                {
                    "day": item_data["last_updated"].strftime("%d/%m/%Y"),
                    "old_price": old_price,
                    "new_price": price,
                }
            )
