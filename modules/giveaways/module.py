...
class Giveaways(commands.GroupCog, name="g", description="Create Custom Giveaways"):
    ...
    async def giveaway_loop(self):
        ...
        giveaways: Dict[int, GiveawayData] = {
            giveaway["_id"]: giveaway
            for giveaway in await self.backend.giveaways.find_many_by_custom(
                {"ended": False}
            )
        }
        ...
        ended_gaw: Dict[int, GiveawayData] = {
            giveaway["_id"]: giveaway
            for giveaway in await self.backend.giveaways.find_many_by_custom(
                {"ended": True}
            )
    ...
    @app_commands.describe(
        ...
        prize="Prize/Quantity of the giveaway",
        time="Duration of the giveaway",
        item="Item to giveaway",
        req_roles="Roles required to enter the giveaway",
        bypass_role="Roles that can bypass the giveaway",
        blacklist_role="Roles that are blacklisted from this giveaway",
        req_level="Level required to enter the giveaway",
        req_weekly="Weekly XP required to enter the giveaway",
        donor="Donor of the giveaway",
        message="Message to accompany the giveaway",
        dank="Dank Memer Giveaway? (Set it to True for Auto Payout Queue)",
        channel_message="Number of Messages required in specific channel (Format: [Channel] [number of messages])",
    )
    @app_commands.autocomplete(item=item_autocomplete)
    @app_commands.rename(
        channel_message="message_requirement",
        prize="quantity",
        req_roles="role_requirement",
        req_level="level_requirement",
        req_weekly="weekly_xp_requirement",
    ...
    async def _start(
        ...
        if time < 60:
            return await interaction.response.send_message(
                "Duration of the giveaway should be atleast 60 seconds!", ephemeral=True
            )
        ...
        if not set(user_role) & set(config["manager_roles"]) or interaction.user.guild_permissions.administrator:
            return await interaction.followup.send(
        ...
        embed = discord.Embed(
            title=f"{config['messages']['gaw']['title']}",
            description=f"{config['messages']['gaw']['description']}",
            color=config["messages"]["gaw"]["color"],
        )
        ...
        raw_timestap = int(
            (datetime.datetime.now() + datetime.timedelta(seconds=time)).timestamp()
        )
        ...
        values = {
            ...
            "donor": donor_name,
            "timestamp": timestamp,
            "host": interaction.user.mention if not donor else donor.mention,
        }
        title_kewrd = {}
        description_kewrd = {}
        ...
        if any(
            ...
            value = ""
            req_role = (
                f"Required: {', '.join([role.mention for role in req_roles])}\n"
                if req_roles
                else None
            )
            bypass_roles = (
                f"Bypass: {', '.join([role.mention for role in bypass_role])}\n"
                if bypass_role
                else None
            )
            blacklist_roles = (
                f"Blacklist: {', '.join([role.mention for role in blacklist_role])}\n"
                if blacklist_role
                else None
            )
        ...
        if message and interaction.guild.me.guild_permissions.manage_webhooks:
            ...
            if not host_webhook:
                ...
                host_webhook = await interaction.channel.create_webhook(
                    name="Giveaway Host", avatar=pfp
                )

            author = donor if donor else interaction.user
            await host_webhook.send(
                content=message,
    ...
    @app_commands.command(
        name="reroll",
        description="Reroll a giveaway Note: Reroll will not Auto Payout Queue Giveaways",
    )
    @app_commands.describe(
    ...
    async def _reroll(
        ...
        if not config:
            return await interaction.followup.send(
                "Giveaways are not enabled in this server!", ephemeral=True
            )
        ...
        if (
            ...
            or "giveaway" not in message.content.lower()
        ):
            return await interaction.response.send_message(
                "This message is not a giveaway!", ephemeral=True
            )
        ...
        if not giveawa_data:
            return await interaction.response.send_message(
                "This giveaway is expired and can't be rerolled!", ephemeral=True
            )
        ...
        if not giveawa_data["ended"]:
            return await interaction.response.send_message(
                "This giveaway has not ended!", ephemeral=True
            )
        giveawa_data["winners"] = winners
        giveawa_data["ended"] = False
        self.bot.dispatch(
            "giveaway_end", giveaway=giveawa_data, config=config, reroll=True
        )
        await interaction.response.send_message(
            "Giveaway rerolled successfully! Make sure to cancel the already queued payouts use `/payout search`",
            ephemeral=True,
        )

    @app_commands.command(name="end", description="End a giveaway")
    @app_commands.describe(message="Message to accompany the end")
    @app_commands.rename(message="message_id")
    async def _end(self, interaction: discord.Interaction, message: str):
        ...
        if not giveaway_data:
            return await interaction.response.send_message(
                "This giveaway Not Found!", ephemeral=True
            )
    ...
    @app_commands.describe(message="Message to accompany the cancel")
    @app_commands.rename(message="message_id")
    async def _cancel(self, interaction: discord.Interaction, message: str):
        ...
        embed.description = (
            f"This giveaway has been cancelled by {interaction.user.mention}!"
        )
        embed.title = None
        ...
        await interaction.response.send_message(
            "Giveaway cancelled successfully!", ephemeral=True
        )

    @app_commands.command(
        name="remove-entry", description="Remove an entry from a giveaway"
    )
    @app_commands.describe(
        message="Message to accompany the removal", user="User to remove"
    )
    @app_commands.rename(message="message_id")
    async def _remove_entry(
        self, interaction: discord.Interaction, message: str, user: discord.Member
    ):
        config = await self.backend.get_config(interaction.guild)
        user_role = [role.id for role in interaction.user.roles]
        if not (set(user_role) & set(config["manager_roles"])):
            return await interaction.response.send_message(
                "You do not have permission to remove entries!", ephemeral=True
            )
        ...
        if str(user.id) not in giveaway_data["entries"].keys():
            return await interaction.response.send_message(
                "This user is not in the giveaway!", ephemeral=True
            )

        if str(user.id) in giveaway_data["entries"].keys():
            del giveaway_data["entries"][str(user.id)]
        if (
            giveaway_data["channel_messages"] != {}
            and str(user.id) in giveaway_data["channel_messages"]["users"].keys()
        ):
            del giveaway_data["channel_messages"]["users"][str(user.id)]
        giveaway_data["banned"].append(user.id)
        await self.backend.update_giveaway(message=message, data=giveaway_data)
        await interaction.response.send_message(
            "Entry removed successfully!", ephemeral=True
        )
        view = Giveaway()
    ...
    async def _list(self, interaction: discord.Interaction):
        giveaways: List[
            GiveawayData
        ] = await self.backend.giveaways.find_many_by_custom(
            {"guild": interaction.guild.id, "ended": False}
        )
        if len(giveaways) == 0:
            return await interaction.response.send_message(
                "No giveaways found in this server!", ephemeral=True
            )
        ...
        await Paginator(interaction=interaction, pages=pages, ephemeral=False).start(
            embeded=True, deffer=True, quick_navigation=False
        )

    @commands.command(
        name="multiplier", description="Set the giveaway multiplier", aliases=["multi"]
    )
    async def _multiplier(self, ctx, user: discord.Member = None):
        ...
        if len(config["multipliers"].keys()) == 0:
            return await ctx.send("This server does not have any multipliers!")
        user_role = [role.id for role in user.roles]
        embed = discord.Embed(color=0x2B2D31, description="@everyone - `1x`\n")
        embed.set_author(
            name=f"{user}'s Multipliers",
    ...
    async def on_message(self, message: discord.Message):
        ...
        if len(giveaways) == 0:
            return
        for giveaway in giveaways:
            if user.id in giveaway["entries"].keys():
                continue
            if giveaway["ended"]:
                continue
            if giveaway["channel_messages"]:
                if message.channel.id != giveaway["channel_messages"]["channel"]:
                    continue
                if str(user.id) not in giveaway["channel_messages"]["users"].keys():
                    ...
                    if time_diff < 8:
                        ...
                        if (
                            giveaway["channel_messages"]["users"][str(user.id)]["count"]
                            >= giveaway["channel_messages"]["count"]
                        ):
                            continue
                        giveaway["channel_messages"]["users"][str(user.id)][
                            "count"
    ...
    async def on_giveaway_end_log(self, giveaway_data: dict):
        ...
        embed.add_field(
            name="Winners",
            value="\n".join(
                [winner.mention for winner in giveaway_data["winners"]]
                if giveaway_data["winners"]
                else ["`None`"]
            ),
        )
        ...
        embed.add_field(
            name="Message", value=f"[Click Here]({giveaway_data['message'].jump_url})"
        )
        ...
        view.add_item(
            discord.ui.Button(
                label="Jump",
                style=discord.ButtonStyle.link,
                url=giveaway_data["message"].jump_url,
            )
        )
    ...
    async def on_giveaway_host(self, data: dict):
        ...
        embed.add_field(
            name="Link",
            value=f"[Click Here](https://discord.com/channels/{data['guild']}/{data['channel']}/{data['_id']})",
        )
        embed.add_field(
            name="Ends At", value=data["end_time"].strftime("%d/%m/%Y %H:%M:%S")
        )
